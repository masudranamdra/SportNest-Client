  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors duration-200">
      ...
      <section className="relative overflow-hidden bg-slate-900 text-white py-16 sm:py-20">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary-950/20 via-slate-900 to-slate-950" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] bg-primary-500/10 blur-[80px] rounded-full" />
        
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          <div className="inline-flex items-center space-x-1 text-xs font-semibold uppercase tracking-wider text-primary-400 bg-primary-500/10 border border-primary-500/20 px-3 py-1 rounded-full">
            <span>Agreement</span>
            <ChevronRight className="h-3 w-3" />
            <span>Terms of Service</span>
          </div>
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-white via-slate-100 to-primary-350 bg-clip-text text-transparent">
            Terms of Service
          </h1>
          ...
          <div className="text-xs text-slate-500 pt-2 font-medium">
            Last Updated: <span className="text-slate-350">{lastUpdated}</span>
          </div>
        </div>
      </section>
            ...
            <aside className="lg:col-span-4 lg:sticky lg:top-24 space-y-6">
              <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-3xl p-6 shadow-sm">
                <h3 className="font-bold text-slate-900 dark:text-white text-base mb-4 flex items-center">
                  <Scale className="h-5 w-5 mr-2 text-primary-500" />
                  Document Outline
                </h3>
                <nav className="space-y-1">
                  {sections.map((sec) => (
                    <a
                      key={sec.id}
                      href={`#${sec.id}`}
                      className="group flex items-center justify-between px-3 py-2 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-primary-50 dark:hover:bg-primary-950/20 hover:text-primary-650 dark:hover:text-primary-400 transition-all"
                    >
                      <span className="truncate">{sec.title.substring(3)}</span>
                      <ChevronRight className="h-4 w-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all text-primary-500" />
                    </a>
                  ))}
                </nav>
              </div>
              ...
              {/* Callout Box */}
              <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/5 dark:from-emerald-950/20 dark:to-teal-950/5 border border-primary-500/20 rounded-3xl p-6 space-y-3">
                ...
                <Link
                  href="/contact"
                  className="inline-flex text-xs font-bold text-primary-650 dark:text-primary-400 hover:text-primary-750 transition-colors"
                >
                  Contact Support →
                </Link>
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <div className="p-2 bg-primary-500/10 text-primary-600 dark:text-primary-400 rounded-xl transition-colors duration-200 group-hover:bg-primary-500 group-hover:text-white">
                            <IconComponent className="h-5 w-5" />
                          </div>
                          <h2 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                            {sec.title}
                          </h2>
                        </div>

  ...
  return (
    <motion.nav 
      ...
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled 
          ? 'py-3 bg-white/90 dark:bg-[#070b19]/90 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/40 shadow-sm dark:shadow-lg dark:shadow-black/20' 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-12">
          ...
          <div className="flex items-center">
            <Link href="/" className="flex justify-center items-center space-x-2.5 group">
              <div className="relative flex items-center justify-center h-10 w-10 bg-gradient-to-tr from-blue-600 to-sky-400 text-white rounded-xl shadow-md shadow-blue-500/10 group-hover:scale-105 group-hover:rotate-6 transition-all duration-300">
                ...
                {/* animate-pulse */}
                <img className="h-5.5 w-5.5" src="https://i.ibb.co.com/fzF0nrZ7/logo1.png" alt="" />
              </div>
          ...
          <div className="hidden md:flex space-x-1 items-center bg-slate-100/80 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-850/60 px-2 py-1.5 rounded-full backdrop-blur-md">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                href={link.path}
              ...
              >
                ...
                {isActive(link.path) && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-white dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/55 rounded-full shadow-sm"
          ...
          <div className="hidden md:flex items-center space-x-3.5">
            ...
            <button
              onClick={toggleTheme}
              className="p-2.5 dark:border-slate-800/40  dark:bg-slate-900/20 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-sky-400 hover:border-blue-500/20 dark:hover:border-sky-500/20 transition-all duration-300"
            ...
            {isAuthenticated ? (
              <div className="relative">
                ...
                <button
                  ...
                  className="flex items-center space-x-2 focus:outline-none p-1 pr-3.5 rounded-full border border-slate-200 dark:border-slate-800/50 hover:border-blue-500/30 dark:hover:border-sky-500/30 bg-slate-100/50 dark:bg-slate-900/30 hover:bg-slate-100 dark:hover:bg-slate-900/60 transition-all"
                >
                  <img
                    src={user?.photoURL}
                    alt={user?.name}
                  ...
                  {dropdownOpen && (
                    <>
                      ...
                      <motion.div 
                        ...
                        className="absolute right-0 mt-3 w-60 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/60 shadow-xl dark:shadow-2xl dark:shadow-black/80 py-2 z-20 overflow-hidden"
                      >
              ...
              <Link
                href="/login"
                className="relative inline-flex items-center justify-center p-0.5 overflow-hidden text-xs font-bold uppercase tracking-wider rounded-xl group bg-gradient-to-br from-blue-650 to-green-500 hover:text-white dark:text-white focus:ring-2 focus:outline-none focus:ring-blue-800 transition-all duration-300"
              >
                <span className="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-slate-950 rounded-[10px] group-hover:bg-opacity-0 text-slate-800 dark:text-white font-extrabold">
                  Login
                </span>
              </Link>
            )}
